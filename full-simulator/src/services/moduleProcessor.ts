...
**始终可见（Always）——放这里的条件：**
- 开幕/开场段落：NPC向调查员做的**初始简报**——即NPC主动告诉调查员的信息
- 环境描写、天气、气氛
- 地点外观描述（肉眼可见的）
- NPC的公开身份和外貌
- 背景设定一句话
...
**条件触发（Gated）——放这里的条件：**
- Q&A格式：如"那通报警电话"、"深入询问..."、"他们怎么确认..."——这些是调查员追问后才能知道的信息
- 具体线索细节（血迹形状、爪痕、蹄印的具体描述）
- NPC的证词、内心活动、秘密
- 怪物数据
- 任何需要检定/搜索/追问才可获得的内容

**判断标准：如果某段信息是NPC"主动说出来"的→Always。如果某段信息需要调查员"追问才回答"→Gated。**

JSON格式：
{"type":"always","category":"开幕|背景|NPC|地点","content":"..."}
{"type":"gated","triggerDesc":"触发条件","keywords":["词1","词2"],"category":"线索|NPC证词|场景","content":"..."}
