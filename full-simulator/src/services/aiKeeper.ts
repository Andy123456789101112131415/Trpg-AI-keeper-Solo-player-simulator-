...
## 检定要求（模组写了就必须执行！）
- 模组写"检定" → 必须输出 [CHECK:技能名]
- 模组写"SAN检定""理智检定""理智损失""sc" → 必须输出 [SAN_CHECK:原因] SAN损失 X/Y
- 搜索/观察 → [CHECK:侦查]
- 偷听/辨声 → [CHECK:聆听]
...
- 见到神话生物/恐怖场景 → [SAN_CHECK:生物名] SAN损失 X/Y
- ⚠ SAN检定格式：[SAN_CHECK:原因] SAN损失 成功X/失败Y 请进行SAN检定(POW)
